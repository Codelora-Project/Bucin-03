...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-2 sm:top-4 z-50 max-w-4xl mx-auto px-2 sm:px-4">
      <nav className="glass-card rounded-full p-1 sm:p-2 border border-amber-300/60 shadow-xl flex items-center justify-between gap-0.5 sm:gap-2">
        {/* Brand / Logo */}
        <button
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="flex items-center gap-1 sm:gap-2 pl-2 sm:pl-3 pr-1 sm:pr-2 py-1 sm:py-1.5 rounded-full hover:bg-amber-100/50 transition-colors shrink-0 cursor-pointer"
          title="Scroll to top"
        >
        ...
        {/* Links Container - Fits 100% on all mobile devices without overflow */}
